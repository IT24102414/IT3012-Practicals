...
class VisualGridHuntGame:
    ...
    def __init__(
        ...
        self.num_traps = max(0, num_traps)
        ...
        available_cells = self.width * self.height - len(self.walls) - len(self.food_positions) - 1
        trap_target = min(self.num_traps, max(0, available_cells))

        while len(self.toxic_traps) < trap_target:
            trap_position = (
                random.randint(0, self.width - 1),
                random.randint(0, self.height - 1),
            )
        ...
        max_opponents = max(0, self.width * self.height - len(self.walls) - len(self.food_positions) - len(self.toxic_traps) - 1)
        opponent_target = min(max(0, num_opponents), max_opponents)

        while len(self.opponents) < opponent_target:
            opponent_position = [
                random.randint(0, self.width - 1),
                random.randint(0, self.height - 1),
            ]
    ...
    def execute_action(self, action: str):
        self.steps += 1
        previous_position = tuple(self.agent_pos)
        ...
        # Apply a toxin penalty only when the agent enters a toxic cell.
        # This prevents repeated penalties while remaining on the same trap.
        if (
            current_position in self.toxic_traps
            ...
            if not self.env.is_done():
                ...
                if isinstance(self.agent, ModelBasedAgent):
                    print(
                        ...
                        f"Percept: {percept} | "
                        f"Action: {action} | "
                        f"Position: {self.agent.position} | "
                        f"Direction: {self.agent.direction} | "
                        f"Visited: {len(self.agent.visited_cells)}"
                    )
                else:
                    print(
                        f"Step {self.env.steps + 1} | "
